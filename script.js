...
fetch(api)
    ...
    .then((data) => {
        console.log(data);
        var listContainerSearch = document.getElementById("listContainerSearch");
        var searchInput = document.getElementById("searchInput");

        function renderSurats(surats) {
            listContainerSearch.innerHTML = "";
            for (var i = 0; i < surats.length; i++) {
                let suratsId = surats[i].id;
                let suratsName = surats[i].surat_name;
                let suratsTerjemahan = surats[i].surat_terjemahan;
                let suratText = surats[i].surat_text;
                let typeSurats = surats[i].type;
                let countAyat = surats[i].count_ayat;
        ...
        renderSurats(data.surats);

        searchInput.addEventListener("input", function () {
            var searchTerm = searchInput.value.toLowerCase();
            var filteredSurats = data.surats.filter(function (surat) {
                return surat.surat_name.toLowerCase().includes(searchTerm);
            });
            renderSurats(filteredSurats);
        });
    });
